...
public class ParseExampleFile {
    ...
    public void countExampleInIteration(){
    	int countOfExamples = 0;
    	int averageNumberofIterations = 0;
    	for(String key: listOfExpts.keySet()){
    		exptContents e = listOfExpts.get(key);
    		System.out.println("");
    		...
    		for(int i=0;i<e.listOfTrials.size();i++){
                ...
                for(int j = 0; j < t.listOfIterations.size(); j++) {                    
                    iteration it = (iteration) (t.listOfIterations.get(j));
                    sum = sum + it.pos.length + it.neg.length;
                    countOfExamples = countOfExamples+sum;                    
                }
                averageNumberofIterations = averageNumberofIterations+t.listOfIterations.size();
                System.out.println("Number of labels for "+ t.listOfIterations.size()+" iterations is  " + sum);
    		}
    	}
    	System.out.println("Average number of lables across trials is  " + countOfExamples/140.0);
    	System.out.println("Average number of iteratiosn across trials is  " + averageNumberofIterations/140.0);    	
    }
    ...
    public static void main(String[] args) {
    	ParseExampleFile m = new ParseExampleFile();
	    m.run();
	    System.out.println(m.listOfExpts);
	    m.countExampleInIteration();
	    //m.printAll();
//	    m.populateIteration();
//	    m.printNumExamples();
    }
